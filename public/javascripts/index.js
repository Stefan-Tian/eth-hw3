...
const transferCoinTo = $('#transferCoinTo');
const transferCoinValue = $('#transferCoinValue');
const transferCoinButton = $('#transferCoinButton');

const getOwner = $('#getOwner');
const getOwnerButton = $('#getOwnerButton');

const transferOwnerTo = $('#transferOwnerTo');
const transferOwnerButton = $('#transferOwnerButton');

const transferAdvancedTo = $('#transferAdvancedTo');
const transferAdvancedValue = $('#transferAdvancedValue');
const transferAdvancedButton = $('#transferAdvancedButton');
...
// when click get owner
getOwnerButton.on('click', async () => {
  if (bankAddress == '') {
    return;
  }
...
transferOwnerButton.on('click', async () => {
  ...
  $.post(
    ...
    result => {
      if (result.events !== undefined) {
        log(result.events.TransferOwnerEvent.returnValues, '轉擁有者成功');
        update.trigger('click');
        doneTransactionStatus();
      } else {
        log(result);
...
// when click advanced transfer
transferAdvancedButton.on('click', async () => {
  if (bankAddress == '') {
    return;
  }
  ...
  $.post(
    ...
    {
      account: nowAccount,
      to: transferAdvancedTo.val(),
      value: parseInt(transferAdvancedValue.val(), 10)
    },
    result => {
      log(`${result}: 進階轉帳成功`);
      doneTransactionStatus();
